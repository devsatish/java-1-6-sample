...
private void buttonCalculateActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_buttonCalculateActionPerformed
    ...
    var numberOfHands = 10000;		// set the number of times the hand is going to be played
    var count_player_one_wins = 0;	// count the number of times player one wins the hand
    var count_player_two_wins = 0;	// count the number of times player two wins the hand
    var tie = 0;					// count the number of times the hand results in a tie
    var ranking_hand_one = 0;		// saves the value of player one current hand in numeric form
    var ranking_hand_two = 0;		// saves the value of player two current hand in numeric form
    var deck = new Deck();			// initializes a new Deck object
    ...
    var best_hand_one = new Hand(); // this Hand object is used to save the best five card combination for player one	
    var best_hand_two = new Hand(); // this Hand object is used to save the best five card combination for player two	

    /** 
     * Set up the hands for the two players
     */
    var handOne = this.textFieldHandOne.getText();
    if("".equals(handOne)){
        ...
        if(handOneArray.length == 1){
	        var putTheCommas = handOneArray[0];
    ...
    var handTwo = this.textFieldHandTwo.getText();
    if("".equals(handTwo)){
        JOptionPane.showMessageDialog(null, """
        ...
        if(handTwoArray.length == 1){
	        var putTheCommas = handTwoArray[0];
     ...
     */
    var board = this.textFieldBoard.getText();
    if("".equals(board)){ 

        for(int i=0; i<numberOfHands; i++){

            var playerOne = new Hand(handPlayerOne);
            var playerTwo = new Hand(handPlayerTwo);
            var deckLeft  = new Deck(deck);
            deckLeft.shuffle();

            var temp_hand 		= new Hand();
            var temp_hand_two 	= new Hand();
            ...
            for(int dc=0; dc<5; dc++){
                ...
                var card_temp1 = playerOne.getCard(dc);
                var deepCopy1 = new Card(card_temp1.getRank(), card_temp1.getSuit());
                best_hand_one.addCard(deepCopy1);  
            }
            for(int dc=0; dc<5; dc++){
                //System.out.println("Entra al for(dc) para deep copy");
                var card_temp2 = playerTwo.getCard(dc);
                var deepCopy2 = new Card(card_temp2.getRank(), card_temp2.getSuit());
            ...
            var random_board = new Card[3];
            for(int b=0; b<3; b++){
                var ayudas = deckLeft.getCard(b);
            ...
            for(int na=0; na<5; na++){
                for(int crd=0; crd<5; crd++){
                    if(crd != na){
                        var card_temp = playerOne.getCard(crd);
                ...
                for(int brd=0; brd<3; brd++){
                    var ayuda_temp = new Card(random_board[brd].getRank(), random_board[brd].getSuit());
                    ...
                    if(temp_hand.getHandRank() > best_hand_one.getHandRank()){
                        ...
                        for(int dc=0; dc<5; dc++){
                            //System.out.println("Entra al for(dc) para deep copy");
                            var best_card_temp = temp_hand.getCard(dc);
                            var deepCopyBest = new Card(best_card_temp.getRank(), best_card_temp.getSuit());
            ...
            for(int na=0; na<5; na++){
                for(int crd=0; crd<5; crd++){
                    if(crd != na){
                        var card_temp_dos = playerTwo.getCard(crd);
                        var deepCopyTwo = new Card(card_temp_dos.getRank(), card_temp_dos.getSuit());
                        temp_hand_two.addCard(deepCopyTwo);
                    }
                }
                for(int brd=0; brd<3; brd++){
                    var ayuda_temp_dos = new Card(random_board[brd].getRank(), random_board[brd].getSuit());
                    ...
                    if(temp_hand_two.getHandRank()> best_hand_two.getHandRank()){
                        ...
                        for(int dc=0; dc<5; dc++){
                            var best_card_temp2 = temp_hand_two.getCard(dc);
                            var deepCopyBest2 = new Card(best_card_temp2.getRank(), best_card_temp2.getSuit());
            ...
            if(best_hand_one.getHandRank() > best_hand_two.getHandRank()){
                ...
                switch(tieBreaker(best_hand_one, best_hand_two)){
                                    case 0 -> tie++;
                                    case 1 -> count_player_one_wins++;
                                    case 2 -> count_player_two_wins++;
                }
            ...
            var hands_tied = (double)Math.round(tie)/100.0;

            System.out.println("Uno gana: "+count_player_one_wins);
            System.out.println("Dos gana: "+count_player_two_wins);
            System.out.println("ties: "+tie);

            var player_one_percentage = (double)Math.round(count_player_one_wins) / 100.0;
            var player_two_percentage = (double)Math.round(count_player_two_wins) / 100.0;
            ...
            if(boardArray.length > 3 ){
                ...
                for(int i=0; i<numberOfHands; i++){
                    ...
                    for(int na=0; na<5; na++){
                        ...
                        for(int brd=0; brd<3; brd++){
                            var ayuda_temp = new Card(boardArray[brd]);
                            temp_hand.addCard(ayuda_temp);
                    ...
                    for(int na=0; na<5; na++){
                        ...
                        for(int brd=0; brd<3; brd++){
                            var ayuda_temp_dos = new Card(boardArray[brd]);
                            temp_hand_two.addCard(ayuda_temp_dos);
                            evaluateHand(temp_hand_two);
    ...
    /**
     * Evaluates the hand of a player and assigns a rank based on the hand's strength.
     * 
    ...
    public void evaluateHand(Hand playerHand){
        if(playerHand.isPair()){
            switch(playerHand.whichKind()){
                ...
                case 2 -> playerHand.setHandRank(2);
                case 3 -> playerHand.setHandRank(3);
                case 4 -> playerHand.setHandRank(6);
                case 6 -> playerHand.setHandRank(7);
            }
        } else {
     ...
     * 
     * @param handOne the first hand to compare
     * @param handTwo the second hand to compare
     * @return an integer indicating the result of the tie break:
     *         0 if it's a tie,
     *         1 if handOne wins,
     *         2 if handTwo wins
     */
    public int tieBreaker(Hand handOne, Hand handTwo){
        return switch(handOne.getHandRank()){
            case 0 -> 0;
            case 1 -> handOne.onePairTieBreak(handTwo);
            case 2 -> handOne.twoPairTieBreak(handTwo);
            case 3 -> handOne.terciaTieBreak(handTwo);
            case 4 -> handOne.straightTieBreak(handTwo);     
            case 5 -> handOne.colorTieBreak(handTwo);    
            case 6 -> handOne.fullTieBreak(handTwo);      
            case 7 -> handOne.pokerTieBreak(handTwo);   
            case 8 -> handOne.straightTieBreak(handTwo);
            default -> 0;
        };
    }
